...
exed). For example, the first node with val = 1, the second node with val = 2,
...
The Graph is connected and all nodes can be visited
...
class _133_CloneGraph {
    ...
    private static final int R = 101;  // 100 nodes at most
    ...
    private static Node cloneGraphBFS(Node root) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Node neighbor : n.neighbors) {
                clone = nodes[neighbor.val];
                if (clone == null) {
                    clone = (nodes[neighbor.val] = new Node(neighbor.val));
