...
started at the beginning of timestamp 3, and "1:end:2" means a function call
...
calls in the program. For example, if a function is called twice,
...
class _636_ExclusiveTimeOfFunctions {
    ...
    private static final class Log {
        ...
        public Log(String log) {
            final String[] segments = log.split(":");
            id = Integer.parseInt(segments[0]);
            isStart = segments[1].equals("start");
            time = Integer.parseInt(segments[2]);
        }
    }

    public static int[] exclusiveTime(int n, List<String> logs) {
        if (n == 0 || n > 100 || logs == null || logs.size() == 0 || logs.size() > 500) return new int[0];

        final Deque<Log> stack = new ArrayDeque<>();
        ...
        for (String line : logs) {
            ...
            if (log.isStart) {
                ...
                final Log top = stack.pop();
                final int duration = (log.time - top.time + 1);
                res[top.id] += duration;
                if (!stack.isEmpty()) {
